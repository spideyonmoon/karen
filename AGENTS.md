...
- **Admin (sudo) commands** (`isAdmin`, hidden from `/help`): `/auth` & `/unauth` with no arg toggle the global admins-only lockdown; with an `<id>` (numeric Telegram ID **or** `@username`) they un-ban / ban a single user — `/unauth <id>` revokes access (gated in `handleMessage`/`handleCallback` via `isBlockedUser`; the user gets one notice then silence), `/auth <id>` restores it. Admins can never be blocked. `/purge` wipes the download cache; `/sys` (`bot/sysstatus.go`) posts a CPU/RAM/disk/uptime card plus a Cloudflare speed test, all read from `/proc` + `syscall.Statfs` (no new deps).
...
- **All JSON state lives in one directory mount: `./bot/state:/app/state`** (`docker-compose.yml`). It holds three files:
  ...
  - `telegram-state.json` — admin lock + per-user `/profile` rip prefs (`UserPrefs map[int64]*UserPrefs`) + per-user bans (`blockedUserIDs` / `blockedUsernames`, set via `/unauth <id>`). This is the **important** data — the only thing the daily backup ships.
