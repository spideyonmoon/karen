...
- **CI compile gate:** `.github/workflows/build-check.yml` runs `go build ./...` (working-dir `bot/`, Go version read from `bot/go.mod`) on **every push and PR, all branches**. Green = it compiles. This is the ONLY automated compile check — there is no test suite.
