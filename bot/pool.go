...
func (p *Pool) UploadToDump(ctx context.Context, path string, m TrackMeta) (dumpID int64, msgID int, err error) {
...
func (p *Pool) uploadToDump(ctx context.Context, path string, m TrackMeta, status *DownloadStatus) (int64, int, error) {
	...
	caption := FormatCaption(m)
	var (
		mid int
		err error
	)
	switch kindOrDefault(m) {
	case "track":
